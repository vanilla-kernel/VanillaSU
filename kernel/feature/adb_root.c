...
static long is_exec_adbd(struct pt_regs *regs)
{
	...
	ret = strncpy_from_user_nofault(buf, fn, sizeof(buf));
	if (ret < 0) {
...
static long setup_ld_preload(struct pt_regs *regs)
{
	...
	ld_library_path_p =
		stackp = ALIGN_DOWN(stackp - sizeof(kLdLibraryPath), 8);
	...
	for (;;) {
		size_t read_count, max_new_env_count, new_env_count = 0;
		bool meet_zero = false;

		tmp_env_p2 = krealloc(tmp_env_p,
				      (env_count + kReadEnvBatch + 2) *
					      kPtrSize,
				      GFP_KERNEL);
		if (tmp_env_p2 == NULL) {
