...
static void
ksu_sulog_set_identity(struct ksu_sulog_event *event,
				     ...
				     char *dst, __u32 dst_len)
{
	...
	ret = strncpy_from_user_nofault(
		dst, (const void __user *)untagged_addr(
		  ...
		  const char __user *const __user *argv_user, gfp_t gfp)
{
	...
	filename_len = ksu_sulog_copy_filename(
		...
		min(remaining, KSU_SULOG_MAX_FILENAME_LEN));
	if (!filename_len)
			     ...
			     gfp_t gfp)
{
	...
	pending = ksu_sulog_capture(KSU_SULOG_EVENT_IOCTL_GRANT_ROOT, NULL, NULL,
				    gfp);
	if (!pending)
		return NULL;
